...
import {
  buildSeatRows,
  COACH_FRAME,
  COACH_PORTRAIT_FRAME,
  resolveCoachSeatLayout,
  splitRowSeats,
  type CoachMapOrientation,
...
function CoachSeatGrid({
  ...
  coach: SeatAvailabilityCoach;
  rows: number[][];
  seatConfiguration: string;
  layout: SeatMapLayoutMetrics;
  seatsByNumber: Map<number, SeatAvailabilitySeat>;
  pendingSeat: number | null;
  activeHold: { seatNumber: number; coachId: string } | null;
  lostSeatNumber: number | null;
  isInteractionDisabled: boolean;
  orientation: CoachMapOrientation;
  onSelectSeat: (seatNumber: number) => void;
}) {
              ...
              <React.Fragment key={`section-${rowIndex}-${sectionIndex}`}>
                {sectionIndex > 0 && (
                  ...
                  >
                    ...
                    <span
                      className={`my-1 whitespace-nowrap font-semibold uppercase tracking-wider text-slate-300 ${
                ...
                <div className="flex items-center" style={{ gap: layout.seatGap }}>
                  {sectionSeats.map((seatNumber) => {
                    ...
                    if (isPortrait) {
                      return <div key={seat.seat_number}>{seatNode}</div>;
                    }
...
function PortraitCoachMap({
  ...
  return (
    <div className="sm:hidden">
      <div className="mb-2 flex flex-col items-center gap-1 text-[10px] font-semibold text-slate-500">
        <span className="uppercase tracking-[0.2em] text-slate-400">Direction of travel</span>
      ...
      <div className="mx-auto flex w-full max-w-xs justify-center">
        <div
          ...
          style={{
            width: COACH_PORTRAIT_FRAME.bodyWidth,
      ...
      <p className="mt-2 text-center text-[10px] text-slate-400">
        Coach shown portrait ({seatConfiguration} per row).
      </p>
    </div>
  );
}

function LandscapeCoachMap({
  coach,
  rows,
  seatConfiguration,
  layout,
...
export function SeatMap({
  ...
  const portraitLayout = resolveCoachSeatLayout(rows.length, seatConfiguration, 'portrait');
  const landscapeLayout = resolveCoachSeatLayout(rows.length, seatConfiguration, 'landscape');
    ...
    <div>
      <div className="overflow-auto rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-100 to-slate-50 px-2 py-3 sm:px-3">
        <div className="mx-auto w-full max-w-4xl">
          <PortraitCoachMap {...sharedProps} layout={portraitLayout} />
          <LandscapeCoachMap {...sharedProps} layout={landscapeLayout} />
          <SeatMapLegend />
        </div>
      </div>
    </div>
